...
const GymCRM = () => {
  ...
  const handleLogin = () => {
    ...
    if (user) {
      setCurrentUser(user);
      setActiveTab(user.isAdmin ? 'schedule' : 'book-classes');
    } else {
  ...
  const toggleOverride = (sessionKey, date) => {
    ...
    setClassOverrides({ ...classOverrides, [key]: !classOverrides[key] });
  };
      ...
      <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center p-4">
        <div className="bg-slate-800 rounded-lg p-8 max-w-md w-full">
          ...
          <div className="space-y-4">
            ...
            />
            <button onClick={handleLogin} className="w-full bg-blue-600 hover:bg-blue-700 px-4 py-3 rounded-lg font-semibold">
              Login
            </button>
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">Fitness Studio {currentUser.isAdmin ? 'Manager' : ''}</h1>
            <p className="text-blue-100">{currentUser.isAdmin ? 'Admin Dashboard' : `Welcome, ${currentUser.name}`}</p>
          </div>
          ...
          <>
                  ...
                  return (
                    <div key={day} className="bg-slate-800 p-6 rounded-lg mb-6">
                      ...
                      {daySessions.map((session) => {
                        const sessionBookings = getSessionBookings(session.key, selectedDate);
                        const count = sessionBookings.length;
                        const myBooking = sessionBookings.find(b => b.memberId === currentUser.id);
                        const overrideKey = `${session.key}-${selectedDate}`;
                            ...
                            {myBooking ? (
                              ...
                              <button
                                ...
                                className={`w-full px-4 py-3 rounded-lg font-semibold ${
                                  currentUser.credits < 6 ? 'bg-slate-600 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'
                                }`}
                              >
            ...
            {activeTab === 'my-bookings' && (
              <div>
                ...
                {bookings
                  .filter(b => b.memberId === currentUser.id && b.date >= selectedDate)
                  .map(booking => {
                    const session = schedule.find(s => s.key === booking.sessionKey);
                    const count = getSessionBookings(booking.sessionKey, booking.date).length;
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-slate-800 rounded-lg p-6 max-w-md w-full">
              ...
              <div className="flex gap-2">
                <button onClick={addBooking} className="flex-1 bg-blue-600 px-4 py-2 rounded-lg">
                  Confirm
                </button>
                <button onClick={() => setShowBookSession(null)} className="flex-1 bg-slate-700 px-4 py-2 rounded-lg">
                  Cancel
                </button>
              </div>
            </div>
        ...
        {showTopUp && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-slate-800 rounded-lg p-6 max-w-md w-full">
              ...
              />
              <div className="flex gap-2">
                <button onClick={handleTopUp} className="flex-1 bg-green-600 px-4 py-2 rounded-lg">
                  Add Credits
                </button>
                <button onClick={() => setShowTopUp(false)} className="flex-1 bg-slate-700 px-4 py-2 rounded-lg">
                  Cancel
                </button>
              </div>
            </div>
